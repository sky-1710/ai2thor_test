...
def teleport_near(c: Controller, agent_id: int, target_x: float, target_z: float,
    ...
    """(x,z) 근처의 reachable 포인트를 찾아 그 y를 사용해 TeleportFull"""
...
def put_on_receptacle(c, agent_id: int, receptacle_obj, place_stationary=True):
    """
    커스텀 빌드 시그니처 대응:
    1) Void PutObject(String objectId, Boolean forceAction=False, Boolean placeStationary=True, Int32 randomSeed=0)
       -> objectId = '책상(리셉터클) objectId'
    2) 폴백: Void PutObject(Single x, Single y, Boolean forceAction=False, Boolean placeStationary=True,
                           Int32 randomSeed=0, Boolean putNearXY=False)
       -> 책상 중심 위쪽 y에 두기
    """
    ...
    try:
        return c.step(
            action="PutObject",
            agentId=agent_id,
            objectId=rec_id,               # ★ receptacleObjectId가 아니라 objectId!
            forceAction=True,
            placeStationary=bool(place_stationary),
            raise_for_failure=True,
        )
    except Exception:
        # 2차 폴백: 좌표로 두기(이 빌드 오버로드는 x,y만 받음; z는 내부에서 인근 지점 추정)
        # 책상 높이보다 살짝 위(y+0.05~0.1)로 시도
        ...
        try:
            return c.step(
                ...
                forceAction=True,
                placeStationary=bool(place_stationary),
                putNearXY=True,            # 근처에 둘 수 있게 힌트
                raise_for_failure=True,
            )
        except Exception:
            # 마지막 시도: placeStationary=False로 느슨하게
            return c.step(
                action="PutObject",
                agentId=agent_id,
                x=float(rec_pos["x"]),
...
def main():
    ...
    teleport_near(c, 0, desk_pos["x"] - 0.60, desk_pos["z"] - 0.50, yaw=0.0,   horizon=18.0)
    teleport_near(c, 1, desk_pos["x"] + 0.60, desk_pos["z"] - 0.50, yaw=180.0, horizon=18.0)
    ...
    for cycle in range(1, 4):
        ...
        teleport_near(c, 1, floor_pos["x"] + 0.25, floor_pos["z"] + 0.25, yaw=180.0, horizon=24.0)
        ensure_facing_and_visible(c, 1, apple, dx=0.10, dz=0.10, yaw=180.0, horizon=20.0)
        safe_pickup(c, 1, apple["objectId"])

        teleport_near(c, 1, desk_pos["x"] + 0.22, desk_pos["z"] - 0.15, yaw=0.0, horizon=6.0)
